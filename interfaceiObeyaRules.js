...
function findRollbyLabelName(nodesiObeya, name, boardid) {
	...
	try {
		...
		for (i in nodesiObeya) { // on scanne la liste de node iObeya
			label = nodesiObeya[i];
			if (label['@class'] === "com.iobeya.dto.BoardLabelDTO" && l_boardid ==nodesiObeya[i].boardid ) { // on recherche le label dans le board cible.
                contentLabel = label.contentLabel.toLocaleUpperCase();
                if (contentLabel === l_name) { // Le roll porte le nom recherché
			        for (j in nodesiObeya ) {
						roll = nodesiObeya[j];
                        if (roll['@class'] === "com.iobeya.dto.BoardRollDTO" && l_boardid == nodesiObeya[j].boardid ) { // on recherche le roll dans le board cible.
...
function placeElement(rollObject, element, status, nodesiObeya, overLappingElements) {
    ...
    try {
		...
		if (isNaN(element.zOrder)) { element.zOrder = maxZOrder(nodesiObeya) + 1; }
		...
		*/
		...
		if (displayType == display_stack) {
		    while (findNoteAtPosition(X, Y, nodesiObeya) != -1 && Y < limit.Y) {
		    	var otherNote = findNoteAtPosition(X, Y, nodesiObeya);
	...
	for (var id = 0; id < nodesiObeya.length; id++){
		...
		if (
			...
			if (chk1) {
				...
				for (var idlabel = 0; idlabel < nodesiObeya.length; idlabel++){
					var iObeyaObject2 = nodesiObeya[idlabel];  
