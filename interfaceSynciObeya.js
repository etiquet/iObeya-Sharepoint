...
function SharePointReloaded(iObeyaConnectedPlatform) {
    ...
    g_notificationID = SP.UI.ModalDialog.showWaitScreenWithNoClose(varTitle, Msg, 120, 700);
// On parse la liste des synchros, pour les actions type update / créate dans sharepoint
    if (iObeyaConnectedPlatform.synclist.length > 0)
        if (iObeyaConnectedPlatform.hasOwnProperty("sharepointToReupdate"))
            if (iObeyaConnectedPlatform.sharepointToReupdate === true) {

                // filtrage les éléments à traiter uniquement create ou update RIDA
                // transformation des éléments creation dans le RIDA par des updates

                for (var iii in iObeyaConnectedPlatform.synclist) {
                    if (!(iObeyaConnectedPlatform.synclist[iii].action === syncType.todo_createRida ||
                            iObeyaConnectedPlatform.synclist[iii].action === syncType.todo_syncRida)
                            )
                        iObeyaConnectedPlatform.synclist[iii].action = todo_nothing; // pas une action rida, on supprime.

                    // On update la liste des éléments à traiter.
                    if (iObeyaConnectedPlatform.synclist[iii].action === syncType.todo_createRida) {
                        iObeyaConnectedPlatform.synclist[iii].action = syncType.todo_syncRida; // transforme les créations en update
                        ...
                        if (ridaObject) // on place l'id rida dans l'objet ( ce qui n'était pas dispo à la création )
                            ...
                            iObeyaConnectedPlatform.synclist[iii].action = todo_nothing; // Prblm, on n'a pas trouvé l'id
                    }

                } //  for ( var iii in iObeyaConnectedPlatform.synclist){
                //
                // on relance une passe de preparation du CAML pour les updates sharepoints
                ...
                iObeyaConnectedPlatform.clientContext.executeQueryAsync(Function.createDelegate(this, function () {
                    SharePointecondCommitQuerySucceeded(iObeyaConnectedPlatform);
                }), Function.createDelegate(this, this.onQueryFailed_test));
                // on lance le timer d'attente
...
// Pas de post traitement, donc on indique au timerfunctionn que c'est fini => fin de la synchronisation.
    iObeyaConnectedPlatform.SharepointToCommit = 0; // flag pour la logique timerfunction

}

// gestion des sémaphores entre threads
function SharePointecondCommitQuerySucceeded(iObeyaConnectedPlatform) {
    console.log("SharePoint Second CommitQuerySucceeded"); // Trace
...
function createNoteFromRida(newNote, iObeyaConnectedPlatform, ridaObj) {
    ...
    if (ridaObj.hasOwnProperty('actor'))
        if (ridaObj.actor instanceof String)
...
function createClonedNote(newNote, iObeyaConnectedPlatform, iObeyaNodeToClone) {
    ...
    if (iObeyaNodeToClone['@class'] === "com.iobeya.dto.BoardNoteDTO") {
        ...
        newNote.props.label0 = iObeyaNodeToClone.props.label0;
        newNote.props.label1 = iObeyaNodeToClone.props.label1;
